...
import org.modelix.model.server.store.RepositoryScopedStoreClient
...
class RepositoriesManager(val client: LocalModelClient) : IRepositoriesManager {
    ...
    private val genericStore: IGenericStoreClient<ObjectInRepository> get() = (client.store as RepositoryScopedStoreClient).client
    ...
    fun getBranchNames(repositoryId: RepositoryId): Set<String> {
        return store.getGenericStore()[branchListKey(repositoryId)]?.lines()?.toSet() ?: emptySet()
    }
    ...
    private fun ensureBranchesAreInList(repository: RepositoryId, branchNames: Set<String>) {
        ...
        val existingBranches = store.getGenericStore()[key]?.lines()?.toSet() ?: emptySet()
    ...
    private fun getVersionHashBlocking(branch: BranchReference): String? {
        return store.runTransaction {
            store.getGenericStore()[branchKey(branch)]
                ?: if (isIsolated(branch.repositoryId) == true) {
                    null
                } else {
                    store.getGenericStore()[legacyBranchKey(branch)]?.also {
                        store.getGenericStore().put(
                            branchKey(branch),
                            it,
                            true,
                        )
    ...
    fun branchKey(branch: BranchReference, isolated: Boolean = isIsolated(branch.repositoryId) ?: true): ObjectInRepository {
        return if (isolated) {
